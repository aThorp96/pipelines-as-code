...
func (p *CustomParams) GetParams(ctx context.Context) (map[string]string, map[string]interface{}, error) {
	...
	for index, value := range *p.repo.Spec.Params {
		...
		} else if paramParsedFromContent && !paramIsStd {
			// Setting to empty string allows the parsedFromComment overrides to
			// set the override value below.
			// If the param is standard, it's initial value will be set later.
